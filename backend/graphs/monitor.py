...
try:
    from langfuse.callback import CallbackHandler  # older Langfuse
except Exception:
    try:
        from langfuse.langchain import CallbackHandler  # newer Langfuse
    except Exception:
...
def begin_trace(trace_name: str, **attrs: Dict[str, Any]) -> None:
    ...
    if current is not None:
        ...
        return
    # Legacy Langfuse SDK path
    if hasattr(client, "trace"):
        tr = client.trace(name=str(trace_name), input=attrs if attrs else None)  # type: ignore[attr-defined]
        _thread_ctx.trace = tr
        return

    # New Langfuse SDK path: store a trace id and create spans against it.
    if getattr(_thread_ctx, "trace_id", None) is None:
        try:
            _thread_ctx.trace_id = client.create_trace_id(seed=str(trace_name))
        except Exception:
            _thread_ctx.trace_id = client.create_trace_id()
    _thread_ctx.trace_name = str(trace_name)

def get_current_trace():
    return getattr(_thread_ctx, "trace", None) or getattr(_thread_ctx, "trace_id", None)
...
def traced_span(name: str, **attrs: Dict[str, Any]):
    ...
    if hasattr(client, "trace"):
        ...
        if parent_trace is None:
            ...
            parent_trace = client.trace(name=str(trace_name), input=attrs)  # type: ignore[attr-defined]
    ...
    # New Langfuse SDK path
    trace_id = getattr(_thread_ctx, "trace_id", None)
    if trace_id is None:
        seed = attrs.get("conversation_id") or attrs.get("trace_id") or "chat_session"
        try:
            trace_id = client.create_trace_id(seed=str(seed))
        except Exception:
            trace_id = client.create_trace_id()
        _thread_ctx.trace_id = trace_id
...
def end_current_trace():
    ...
    if getattr(_thread_ctx, "trace_id", None) is not None:
        _thread_ctx.trace_id = None
    try:
        client.flush()
    except Exception:
        pass
