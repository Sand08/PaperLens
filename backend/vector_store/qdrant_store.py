...
class QdrantStore:
    ...
    def similarity_search(
        ...
        try:
            ...
            # qdrant-client>=1.15 removed `client.search`, which older wrappers still call.
            if "has no attribute 'search'" in str(e):
                try:
                    return self._client_similarity_search(query, embedder, k=k)
                except Exception as inner:
                    st.error(f"❌ Search failed: {inner}")
                    return []
            st.error(f"❌ Search failed: {e}")
            return []

    def _client_similarity_search(
        self,
        query: str,
        embedder: Embeddings,
        k: int = 5,
    ...
    def as_retriever(self, **kwargs):
        ...
        # For Qdrant, use a compatibility retriever to avoid client API drift.
        from backend.embeddings.model_registry import get_embedder
        ...
        return _CompatRetriever(self, emb, search_kwargs)
