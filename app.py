...
def load_config():
    ...
    def get_secret(key, default=None):
        ...
        if key == "LANGFUSE_PUBLIC_KEY":
            return secrets.get("langfuse", {}).get("public_key", default)
        elif key == "LANGFUSE_SECRET_KEY":
            return secrets.get("langfuse", {}).get("secret_key", default)
        return default
...
with st.sidebar:
    ...
    st.session_state["llm_sel"] = st.selectbox(
        "LLM Model",
        MODEL_OPTIONS,
        index=MODEL_OPTIONS.index(st.session_state["llm_sel"]) if st.session_state["llm_sel"] in MODEL_OPTIONS else 0,
